...
public abstract class BaseDialog {
        ...
        ActivityLifecycleImpl.init(context, new ActivityLifecycleImpl.onActivityResumeCallBack() {
            ...
            public void getActivity(Activity activity) {
                try {
                    ...
                    error("HeadDialog.init: 初始化异常，找不到Activity的根布局");
                }
            }
        });
    }
    ...
    protected void beforeShow() {
        if (getContext() == null) {
            error("HeadDialog 未初始化。\n请检查是否在启动对话框前进行初始化操作，使用以下代码进行初始化：\nHeadDialog.init(context);");
        }
        if (style.styleVer != DialogStyle.styleVer) {
            error("HeadDialog 所引用的 Style 不符合当前适用版本：" + DialogStyle.styleVer + " 引入的 Style(" + style.getClass().getSimpleName() + ") 版本" + style.styleVer);
        }
    }
