...
import com.head.titlebar.statusbar.StatusBarUtils;
import com.head.titlebar.utils.ScreenUtils;


/**
 * 类名称：CommonTitleBar.java <br/>
 * 类描述：通用标题栏<br/>
 * 创建人：shuwen <br/>
 * 创建时间：2020-01-17 14:52 <br/>
 */
...
public class HeadTitleBar extends RelativeLayout
    ...
    private void loadAttributes(Context context, AttributeSet attrs) {
        ...
        TypedArray array =
                context.obtainStyledAttributes(attrs, R.styleable.HeadTitleBar);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            // notice 未引入沉浸式标题栏之前,隐藏标题栏撑起布局
            fillStatusBar = array
                    .getBoolean(R.styleable.HeadTitleBar_fillStatusBar, true);
        }
        titleBarColor = array.getResourceId(R.styleable.HeadTitleBar_titleBarColor,R.color.default_title);
        titleBarHeight =
                (int) array.getDimension(R.styleable.HeadTitleBar_titleBarHeight,
                        ScreenUtils.dp2PxInt(context, 44));
        statusBarColor =
                array.getResourceId(R.styleable.HeadTitleBar_statusBarColor,R.color.default_title);
        statusBarMode =
                array.getInt(R.styleable.HeadTitleBar_statusBarMode, 0);

        showBottomLine =
                array.getBoolean(R.styleable.HeadTitleBar_showBottomLine, true);
        bottomLineColor =
                array.getColor(R.styleable.HeadTitleBar_bottomLineColor,
                        Color.parseColor("#dddddd"));
        bottomShadowHeight =
                array.getDimension(R.styleable.HeadTitleBar_bottomShadowHeight,
                        ScreenUtils.dp2PxInt(context, 0));

        leftType =
                array.getInt(R.styleable.HeadTitleBar_leftType, TYPE_LEFT_NONE);
        if (leftType == TYPE_LEFT_TEXTVIEW) {
            leftText = array.getString(R.styleable.HeadTitleBar_leftText);
            leftTextColor = array.getColor(
                    R.styleable.HeadTitleBar_leftTextColor,
                    getResources().getColor(R.color.titlebar_comm_text_selector));
            leftTextSize =
                    array.getDimension(R.styleable.HeadTitleBar_leftTextSize,
                            ScreenUtils.dp2PxInt(context, 16));
            leftDrawable =
                    array.getResourceId(R.styleable.HeadTitleBar_leftDrawable, 0);
            leftDrawablePadding = array.getDimension(
                    R.styleable.HeadTitleBar_leftDrawablePadding,
                    5);
        } else if (leftType == TYPE_LEFT_IMAGEBUTTON) {
            leftImageResource = array.getResourceId(
                    R.styleable.HeadTitleBar_leftImageResource,
                    R.drawable.comm_titlebar_reback_selector);
        } else if (leftType == TYPE_LEFT_CUSTOM_VIEW) {
            leftCustomViewRes = array
                    .getResourceId(R.styleable.HeadTitleBar_leftCustomView, 0);
        }

        rightType =
                array.getInt(R.styleable.HeadTitleBar_rightType, TYPE_RIGHT_NONE);
        if (rightType == TYPE_RIGHT_TEXTVIEW) {
            rightText = array.getString(R.styleable.HeadTitleBar_rightText);
            rightTextColor = array.getColor(
                    R.styleable.HeadTitleBar_rightTextColor,
                    getResources().getColor(R.color.titlebar_comm_text_selector));
            rightTextSize =
                    array.getDimension(R.styleable.HeadTitleBar_rightTextSize,
                            ScreenUtils.dp2PxInt(context, 16));
        } else if (rightType == TYPE_RIGHT_IMAGEBUTTON) {
            rightImageResource = array.getResourceId(
                    R.styleable.HeadTitleBar_rightImageResource,
                    0);
        } else if (rightType == TYPE_RIGHT_CUSTOM_VIEW) {
            rightCustomViewRes = array
                    .getResourceId(R.styleable.HeadTitleBar_rightCustomView, 0);
        }

        centerType = array.getInt(R.styleable.HeadTitleBar_centerType,
                TYPE_CENTER_NONE);
        if (centerType == TYPE_CENTER_TEXTVIEW) {
            centerText = array.getString(R.styleable.HeadTitleBar_centerText);
            centerTextColor =
                    array.getColor(R.styleable.HeadTitleBar_centerTextColor,
                            Color.parseColor("#333333"));
            centerTextSize =
                    array.getDimension(R.styleable.HeadTitleBar_centerTextSize,
                            ScreenUtils.dp2PxInt(context, 18));
            centerTextMarquee = array
                    .getBoolean(R.styleable.HeadTitleBar_centerTextMarquee, true);
            centerSubText =
                    array.getString(R.styleable.HeadTitleBar_centerSubText);
            centerSubTextColor =
                    array.getColor(R.styleable.HeadTitleBar_centerSubTextColor,
                            Color.parseColor("#666666"));
            centerSubTextSize =
                    array.getDimension(R.styleable.HeadTitleBar_centerSubTextSize,
                            ScreenUtils.dp2PxInt(context, 11));
        } else if (centerType == TYPE_CENTER_SEARCHVIEW) {
            centerSearchEditable = array.getBoolean(
                    R.styleable.HeadTitleBar_centerSearchEditable,
                    true);
            centerSearchBgResource =
                    array.getResourceId(R.styleable.HeadTitleBar_centerSearchBg,
                            R.drawable.comm_titlebar_search_gray_shape);
            centerSearchRightType =
                    array.getInt(R.styleable.HeadTitleBar_centerSearchRightType,
                            TYPE_CENTER_SEARCH_RIGHT_VOICE);
        } else if (centerType == TYPE_CENTER_CUSTOM_VIEW) {
            centerCustomViewRes = array
                    .getResourceId(R.styleable.HeadTitleBar_centerCustomView, 0);
        }
    ...
    private void initGlobalViews(Context context) {
        ...
        if (fillStatusBar && transparentStatusBar) {
            ...
            viewStatusBarFill.setBackgroundResource(statusBarColor);
            LayoutParams statusBarParams =
                    new LayoutParams(MATCH_PARENT, statusBarHeight);
...
//        }else {
        rlMain.setBackgroundResource(titleBarColor);
//        }

//        rlMain.setBackgroundColor(titleBarColor);
//        if (titleBarColor instanceof ColorDrawable) {
////            rlMain.setBackgroundColor(titleBarColor);
//
//        } else {
//            rlMain.setBackground(titleBarColor);
//        }
        LayoutParams mainParams =
                new LayoutParams(MATCH_PARENT, titleBarHeight);
        if (fillStatusBar && transparentStatusBar) {
    ...
    /**
     * 初始化主视图左边部分 -- add: adaptive RTL
     *
     * @param context 上下文
     */
    private void initMainLeftViews(Context context) {
        LayoutParams leftInnerParams =
        ...
        if (leftType == TYPE_LEFT_TEXTVIEW) {
            ...
            if (leftDrawable != 0) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                    ...
                    tvLeft.setCompoundDrawablesWithIntrinsicBounds(leftDrawable,
                            0,
                            0,
                            0);
                }
            ...
            // 初始化自定义View
            viewCustomLeft = LayoutInflater.from(context)
                    .inflate(leftCustomViewRes, rlMain, false);
    ...
    /**
     * 初始化主视图右边部分 -- add: adaptive RTL
     *
     * @param context 上下文
     */
    private void initMainRightViews(Context context) {
        LayoutParams rightInnerParams =
                new LayoutParams(WRAP_CONTENT, MATCH_PARENT);
    ...
    private void initMainCenterViews(Context context) {
        if (centerType == TYPE_CENTER_TEXTVIEW) {
            ...
            // 设置跑马灯效果
            tvCenter.setMaxWidth(
                    (int) (ScreenUtils.getScreenPixelSize(context)[0] * 3 / 5.0));
            ...
            LayoutParams progressParams =
                    new LayoutParams(progressWidth, progressWidth);
            progressParams.addRule(RelativeLayout.CENTER_VERTICAL);
            progressParams.addRule(RelativeLayout.START_OF,
                    llMainCenter.getId());
            ...
            tvCenterSub.setTextSize(TypedValue.COMPLEX_UNIT_PX,
                    centerSubTextSize);
            ...
            if (leftType == TYPE_LEFT_TEXTVIEW) {
                ...
                centerParams.addRule(RelativeLayout.END_OF,
                        viewCustomLeft.getId());
                centerParams.setMarginStart(PADDING_5);
            } else {
                centerParams.setMarginStart(PADDING_12);
            }
            ...
            if (rightType == TYPE_RIGHT_TEXTVIEW) {
                ...
                centerParams.addRule(RelativeLayout.START_OF,
                        viewCustomRight.getId());
                centerParams.setMarginEnd(PADDING_5);
            } else {
                centerParams.setMarginEnd(PADDING_12);
            }
            ...
            LayoutParams searchParams =
                    new LayoutParams(searchIconWidth, searchIconWidth);
            ...
            if (centerSearchRightType == TYPE_CENTER_SEARCH_RIGHT_VOICE) {
                ...
                ivVoice
                        .setImageResource(R.drawable.comm_titlebar_delete_normal);
            ...
            etSearchHint.setHint(
                    getResources().getString(R.string.titlebar_search_hint));
            ...
            LayoutParams centerCustomParams =
                    new LayoutParams(WRAP_CONTENT, MATCH_PARENT);
            ...
            // if (leftType == TYPE_LEFT_TEXTVIEW) {
            // centerCustomParams.addRule(RelativeLayout.END_OF,
            // tvLeft.getId());
            // } else if (leftType == TYPE_LEFT_IMAGEBUTTON) {
            // centerCustomParams.addRule(RelativeLayout.END_OF,
            // btnLeft.getId());
            ...
            // }
            // if (rightType == TYPE_RIGHT_TEXTVIEW) {
            // centerCustomParams.addRule(RelativeLayout.START_OF,
            // tvRight.getId());
            // } else if (rightType == TYPE_RIGHT_IMAGEBUTTON) {
            // centerCustomParams.addRule(RelativeLayout.START_OF,
            // btnRight.getId());
            ...
            if (centerSearchRightType == TYPE_CENTER_SEARCH_RIGHT_VOICE) {
                if (TextUtils.isEmpty(s)) {
                    ...
                    ivVoice.setImageResource(
                            R.drawable.comm_titlebar_delete_normal);
                }
            } else {
                if (TextUtils.isEmpty(s)) {
                    ivVoice.setVisibility(View.GONE);
    ...
    public void setCenterView(View centerView) {
        ...
        LayoutParams centerInnerParams =
                new LayoutParams(WRAP_CONTENT, MATCH_PARENT);
    ...
    public void setDoubleClickListener(
            OnTitleBarDoubleClickListener doubleClickListener) {
