...
public class HeadTabBar extends View implements ViewPager.OnPageChangeListener {
    ...
    // External background view for the head
    ...
    private boolean animationSwitch = true;

    public HeadTabBar(final Context context) {
        this(context, null);
    }

    public HeadTabBar(final Context context, final AttributeSet attrs) {
        this(context, attrs, 0);
    }

    @SuppressWarnings({"ResourceAsColor", "ResourceType"})
    public HeadTabBar(final Context context, final AttributeSet attrs, final int defStyleAttr) {
        ...
        final TypedArray typedArray =
                context.obtainStyledAttributes(attrs, R.styleable.HeadTabBar);
        try {
            setIsTitled(typedArray.getBoolean(R.styleable.HeadTabBar_head_titled, false));
            setIsBadged(typedArray.getBoolean(R.styleable.HeadTabBar_head_badged, false));
            setIsScaled(typedArray.getBoolean(R.styleable.HeadTabBar_head_scaled, true));
            setIsTinted(typedArray.getBoolean(R.styleable.HeadTabBar_head_tinted, true));
            setIsSwiped(typedArray.getBoolean(R.styleable.HeadTabBar_head_swiped, true));
            setTitleSize(
                    typedArray.getDimension(R.styleable.HeadTabBar_head_title_size, AUTO_SIZE)
            );
            setIsBadgeUseTypeface(
                    typedArray.getBoolean(
                            R.styleable.HeadTabBar_head_badge_use_typeface,
                            false
                    )
            );
            ...
            setTitleMode(
                    typedArray.getInt(
                            R.styleable.HeadTabBar_head_title_mode, TitleMode.ALL_INDEX
                    )
            );
            setBadgeSize(
                    typedArray.getDimension(R.styleable.HeadTabBar_head_badge_size, AUTO_SIZE)
            );
            setBadgePosition(
                    typedArray.getInt(
                            R.styleable.HeadTabBar_head_badge_position,
                            BadgePosition.RIGHT_INDEX
                    )
            );
            setBadgeGravity(
                    typedArray.getInt(
                            R.styleable.HeadTabBar_head_badge_gravity, BadgeGravity.TOP_INDEX
                    )
            );
            setBadgeBgColor(
                    typedArray.getColor(R.styleable.HeadTabBar_head_badge_bg_color, AUTO_COLOR)
            );
            setBadgeTitleColor(
                    typedArray.getColor(
                            R.styleable.HeadTabBar_head_badge_title_color, AUTO_COLOR
                    )
            );

            setTypeface(typedArray.getString(R.styleable.HeadTabBar_head_typeface));
            setInactiveColor(
                    typedArray.getColor(
                            R.styleable.HeadTabBar_head_inactive_color, DEFAULT_INACTIVE_COLOR
                    )
            );
            setActiveColor(
                    typedArray.getColor(
                            R.styleable.HeadTabBar_head_active_color, DEFAULT_ACTIVE_COLOR
                    )
            );
            setBgColor(
                    typedArray.getColor(
                            R.styleable.HeadTabBar_head_bg_color, DEFAULT_BG_COLOR
                    )
            );
            setAnimationDuration(
                    typedArray.getInteger(
                            R.styleable.HeadTabBar_head_animation_duration,
                            DEFAULT_ANIMATION_DURATION
                    )
            );
            setCornersRadius(
                    typedArray.getDimension(R.styleable.HeadTabBar_head_corners_radius, 0.0F)
            );
            setIconSizeFraction(
                    typedArray.getFloat(
                            R.styleable.HeadTabBar_head_icon_size_fraction,
                            AUTO_SCALE
                    )
            );
            ...
            if (isInEditMode()) {
                ...
                try {
                    final int previewColorsId = typedArray.getResourceId(
                            R.styleable.HeadTabBar_head_preview_colors, 0
                    );
...
//            mIsTitled = false;
    ...
    protected void onDraw(final Canvas canvas) {
        // Get height of head with badge on nor
        ...
        for (int i = 0; i < mModels.size(); i++) {
            ...
            // Check if we handle models from touch on head or from ViewPager
    ...
    // Method to transform current fraction of head and position
    ...
    // Method to transform last fraction of head and position
    ...
    // Method to transform others fraction of head and position
    ...
    protected float clampValue(final float value) {
        return Math.max(
                Math.min(value, HeadTabBar.MAX_FRACTION), HeadTabBar.MIN_FRACTION
        );
    }

    // Hide head with animation
    public void hide() {
    ...
    // Show head with animation
    public void show() {
        if (mBehavior != null) mBehavior.resetOffset(this, true);
        else scrollUp();
    }

    // Hide head or bg on scroll down
